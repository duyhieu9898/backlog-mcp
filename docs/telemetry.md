...
- `claude` chạy với `--strict-mcp-config --mcp-config <tmp>/mcp.json` (file tạo cho từng run trong thư mục tạm, không ghi vào workspace; chỉ có server `backlog` = `uv --project <repo> run backlog-mcp-server`) và `--disallowedTools Bash Edit Write NotebookEdit WebFetch WebSearch Task Agent`. Ở permission mode `auto` `--allowedTools` không chặn tool nên phải cô lập bằng MCP config; hook/skill global vẫn giữ (D9). `agy` chạy **không** `--sandbox` (sandbox làm hỏng mọi lệnh terminal: `connecting to sandbox server … connection reset`); an toàn dựa vào env đã bỏ `BACKLOG_*`, fake backend và mã `OOP-9xxxxx`. agy không có `--strict-mcp-config` nên runner tạm `agy mcp disable` mọi MCP server global đang bật (trừ `backlog`) trong lúc chạy batch và `enable` lại khi kết thúc, kể cả khi lỗi. Tool bị từ chối nằm ở `deniedTools`.
- Server `backlog` của agy là cấu hình global (`agy mcp list`) chạy từ thư mục repo này: sửa code trong lúc eval agy sẽ lẫn vào kết quả. Muốn vừa eval agy vừa sửa code thì để thư mục repo ở commit cần đo và sửa code trong một git worktree khác.
- Runner đọc stdout theo luồng và dừng agent ngay khi có event `result` (agy không tự thoát, sống tới `--print-timeout`); `--timeout` + 30 s là chốt chặn. `agentOk` của agy chỉ đúng khi `status == SUCCESS` **và** có ít nhất một bước. Lượt fail do môi trường có `envError` (`service_unavailable`, `sandbox`, `timeout`, `agent_failed`) và được đếm riêng ở cột `EnvErr` trong `SUMMARY.md`, không cộng vào "Lý do fail phổ biến".
